...
if($_REQUEST['attendance'] && ($_POST['attendance'] || $_REQUEST['ajax']) && AllowEdit())
{
	foreach($_REQUEST['attendance'] as $student_id=>$values)
	{
		foreach($values as $period=>$columns)
		{
			...
			if($current_RET[$student_id][$p_ids[1]])
			{
				...
				$sql = substr($sql,0,-1) . ' WHERE SCHOOL_DATE=\''.$date.'\' AND COURSE_PERIOD_ID=\''.$p_ids[1].'\' AND STUDENT_ID=\''.$student_id.'\''.$extra_sql;
				DBQuery($sql);
			}
			else
			{
...
//				$period_id = $period_id[1]['PERIOD_ID'];
				...
				$values = '\''.$student_id.'\',\''.$date.'\',\''.$p_ids[1].'\',\''.$current_mp.'\',\''.$period_id.'\',\'Y\',';
	...
	if(isset($_REQUEST['student_id']) )
{
	...
	DrawHeaderHome('Selected Student: '.$RET[1]['FIRST_NAME'].'&nbsp;'.($RET[1]['MIDDLE_NAME']?$RET[1]['MIDDLE_NAME'].' ':'').$RET[1]['LAST_NAME'].'&nbsp;'.' (<A HREF=Side.php?student_id=new&modcat='.$_REQUEST['modcat'].'><font color=red>Search Again</font></A>) | <A HREF=Modules.php?modname='.$_REQUEST['modname'].'&search_modfunc=list&next_modname=students/Student.php&codes[]='.$_SESSION[code][0].'&ajax=true&bottom_back=true&month_date='.$_REQUEST[month_date].'&day_date='.$_REQUEST[day_date].'&year_date='.$_REQUEST[year_date].' target=body  style="text-decoration:none"><strong>Back to Student List</strong></A>');
	#-----------------------------------------------------newly added attendance code and the date in back to list--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
